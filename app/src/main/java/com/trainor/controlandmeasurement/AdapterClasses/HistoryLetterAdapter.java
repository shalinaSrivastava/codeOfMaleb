...
public class HistoryLetterAdapter extends RecyclerView.Adapter<HistoryLetterAdapter.MyViewHolder> {
    ...
    public HistoryLetterAdapter(Context con, ViewModel _viewModel) {
        this.context = con;
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        holder.ll_open.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                List<ImageEntity> imageEntities = viewModel.getImagesToUpload(adminID, _entityInfo.letterID, _entityInfo.measurePointID);
                if (imageEntities != null && imageEntities.size() > 0) {
                    AlertDialogManager.showDialog(context, context.getResources().getString(R.string.ok), context.getResources().getString(R.string.cancel), context.getResources().getString(R.string.image_upload), _entityInfo.measurePointID + " : " + imageEntities.size() + " " + context.getResources().getString(R.string.image_could_not_uploaded), true, new IClickListener() {
                        @Override
                        public void onClick() {
    ...
    public void setDefaultValues(LetterEntity _entity) {
        ...
        if (_entity.disconnectTime.contains("-2.0")) {
            spManager.saveVariablerValueByKeyName("Disablement", "0", variableEditor);
        } else {
           /* int _pos = XMLParser.getDisconnectName(_entity.disconnectTime, "disconnectTimes", "Seconds");
            if (_pos == 0) {
                _pos = XMLParser.getDisconnectName(_entity.disconnectTime, "disconnectTimes", "Name");
            }
            spManager.saveVariablerValueByKeyName("Disablement", _pos + "", variableEditor);*/
